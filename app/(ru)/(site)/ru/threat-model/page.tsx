...
const ADVERSARIES = [
  ...
  },
  {
    ...
    cannot: 'Прочитать отрэтченную часть переписок — те ключи были случайными и device-local',
    defence: 'Фраза не покидает устройство и никогда не передаётся',
    residual: 'Идентичность, signed prekey и PQ-prekey выводятся из фразы, а одноразовые prekey не публикуются — поэтому первое сообщение каждой переписки восстанавливается из фразы и вечного конверта в сети. Отзыва тоже нет',
  },
  {
  ...
  },
  {
    ...
    defence: 'Prekey-bundle подписаны кошельком и опубликованы в сети, поэтому реестр не может подменить ключ аккаунта',
    residual: 'Нет ни safety number, ни отпечатка для сверки — убедиться, что аккаунт принадлежит нужному человеку, можно только вне приложения и без его помощи',
  },
  {
    name: 'Apple',
