...
const FAQS = [
  {
    ...
    a: 'Один. Fee worker оплачивает комиссию сети Solana за каждую транзакцию, чтобы вам не приходилось держать SOL. На анонимном пути он видит действительный непотраченный слепой токен и транзакцию — ни аккаунта, ни открытого текста, ни получателя. Базы аккаунтов и хранилища сообщений нет.',
  },
  {
      ...
      <div className="mt-14 max-w-3xl">
        <Prose>
          ...
          <p>
            ...
            один токен. Воркер проверяет, что токен действителен и не потрачен, но не может связать
            его ни с покупкой, ни с другим токеном, ни с аккаунтом — на анонимном пути аккаунт не
            передаётся вовсе. Legacy-путь с аккаунтом остаётся только для публикации публичного
            ника, который идентифицирует по своей природе.
          </p>
